...
const BRAND = { name: "Moving2Bulgaria", tagline: "Clear, practical guides to make your Bulgaria move simple.", cta: "Start your move" };
const DOMAIN = { url: "moving2bulgaria.com" };
const CONTACT = { email: "hello@moving2bulgaria.com", phone: "+44 7394 048818", location: "UK • Bulgaria" };
...
const products = [
  { id: "visa-irish", title: "Visa & Residency (Irish/UK citizens)", blurb: "Step-by-step process, timelines, documents, and costs. Includes office locations and sample forms.", price: "£19", buyUrl: "#visa-checkout", highlights: ["Residency card steps","Translations & apostilles","Pitfalls to avoid"] },
  { id: "property", title: "House-Hunting & Buying Guide", blurb: "Where to search, agent scripts, legal checks, and a full purchase checklist (BG & EN).", price: "£24", buyUrl: "#property-checkout", highlights: ["Region cheat-sheet","Lawyer brief","Offer template"] },
  { id: "car-import", title: "UK→BG Car Import & Registration", blurb: "Transport options, customs, insurance, taxes, and re-registration (with example costs).", price: "£14", buyUrl: "#car-checkout", highlights: ["Doc list","Plates & fees","Timeline"] },
  { id: "setup-pack", title: "New-Life Setup Pack (Bundle)", blurb: "Banking, SIM, utilities, healthcare, driving licences, and local translations in one pack.", price: "£39", buyUrl: "#bundle-checkout", highlights: ["All starter docs","Contact directory","Budget templates"], badge: "Best value" },
];
    ...
    <div className="min-h-screen bg-white text-gray-900">
      <header className="sticky top-0 z-30 backdrop-blur supports-[backdrop-filter]:bg-white/75 bg-white/90 border-b">
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center justify-between">
          ...
          <a href="#products" className="rounded-2xl px-4 py-2 text-sm font-medium bg-[#00966E] text-white hover:bg-[#007857] shadow">{BRAND.cta}</a>
        </div>
      </header>
      ...
      <section className="relative overflow-hidden">
        <div className="mx-auto max-w-7xl px-4 py-16 grid md:grid-cols-2 gap-10 items-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-6">
            <h1 className="text-4xl md:text-5xl font-extrabold leading-tight">Make your move to Bulgaria <span className="text-[#006B4F]">simple</span></h1>
            <p className="text-lg text-gray-600">Real-world, no-nonsense guides based on lived experience. Avoid costly mistakes, save time, and land on your feet.</p>
            <div className="flex flex-wrap gap-3">
              <a href="#products" className="rounded-xl px-5 py-3 bg-[#00966E] text-white font-medium hover:bg-[#007857] shadow">Browse Guides</a>
              <a href="#youtube" className="rounded-xl px-5 py-3 bg-gray-100 hover:bg-gray-200 font-medium">Watch on YouTube</a>
            </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {products.map((p) => (
              <div key={p.id} className="group rounded-3xl bg-white border shadow-sm hover:shadow-md transition overflow-hidden">
                {p.badge && (<div className="absolute"><span className="ml-4 mt-4 inline-block rounded-full bg-[#00966E] text-white text-xs px-3 py-1">{p.badge}</span></div>)}
                <div className="p-6">
                  <h3 className="font-bold text-lg leading-tight">{p.title}</h3>
                  <p className="text-sm text-gray-600 mt-2">{p.blurb}</p>
                  <ul className="mt-4 space-y-1 text-sm text-gray-700">
                    {p.highlights?.map((h, i) => (<li key={i} className="flex gap-2 items-start"><span className="mt-1 h-1.5 w-1.5 rounded-full bg-[#00966E]"/>{h}</li>))}
                  </ul>
                </div>
        ...
        <div className="mx-auto max-w-7xl px-4">
          ...
          <div className="flex gap-4 overflow-x-auto snap-x snap-mandatory pb-2">
            {YOUTUBE.latest.map((vid, i) => (
              <a key={i} href={vid ? ('https://www.youtube.com/watch?v=' + vid) : YOUTUBE.channelUrl} target="_blank" rel="noopener noreferrer" className="min-w-[280px] max-w-[320px] snap-start group rounded-2xl border overflow-hidden bg-white hover:shadow-md transition">
                <div className="aspect-video w-full bg-gray-100">
                  {vid ? (<img src={'https://i.ytimg.com/vi/' + vid + '/hqdefault.jpg'} alt="YouTube thumbnail" className="h-full w-full object-cover" />) : (<div className="h-full w-full grid place-items-center text-gray-400">Add video ID</div>)}
                </div>
        ...
        <div className="mx-auto max-w-7xl px-4 grid lg:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <div className="mt-6 flex gap-3">
              <a href={YOUTUBE.channelUrl} target="_blank" className="rounded-xl px-5 py-3 bg-gray-900 text-white font-medium hover:opacity-90">Visit Channel</a>
              <a href={YOUTUBE.channelUrl + (YOUTUBE.channelUrl.includes('?') ? '&' : '?') + 'sub_confirmation=1'} target="_blank" rel="noopener noreferrer" className="rounded-xl px-5 py-3 bg-[#00966E] text-white font-medium hover:bg-[#007857] shadow">Subscribe</a>
              <a href="#faq" className="rounded-xl px-5 py-3 bg-gray-100 hover:bg-gray-200 font-medium">Read FAQ</a>
            </div>
          </div>
          <div>
            {YOUTUBE.embed && YOUTUBE.featuredVideoId ? (
              <div className="aspect-video w-full rounded-3xl overflow-hidden border shadow-sm">
                <iframe className="h-full w-full" src={"https://www.youtube-nocookie.com/embed/" + YOUTUBE.featuredVideoId + "?rel=0&modestbranding=1&playsinline=1&enablejsapi=0&autoplay=1&mute=1"} title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen></iframe>
              </div>
            ) : (
              <div className="w-full rounded-3xl border bg-gray-50 p-6 shadow-sm">
                ...
                <a href={YOUTUBE.channelUrl} target="_blank" rel="noopener noreferrer" className="mt-4 inline-block rounded-xl px-5 py-3 bg-gray-900 text-white font-medium hover:opacity-90">Open channel</a>
              </div>
            )}
            <p className="text-xs text-gray-500 mt-2">Embeds are enabled in production.</p>
          </div>
        </div>
      </section>
